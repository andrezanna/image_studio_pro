              ...
              onPressed: () {
                if (widget.isImage) {
                  ...
                  screenshotController
                    ...
                    print(file.path);
                    file.writeAsBytesSync(binaryIntList);
                    final result = await _blendMedia(
                        overlay: file, mediaFile: widget.defaultMedia);
...
/*
                  ...
                  BottomBarContainer(
                    ...
                    ontap: () {
                      ...
                      getemojis.then((value) {
                        if (value['name'] != null) {
                          final box = globalKey.currentContext
                          ...
                          offsets.add(Offset(box.size.width / 2 - 84,
                              box.size.height / 2 - 84));
                                            ...
                                            setState(() {
                                              if (oldScale == -1) {
                                                ...
                                                offsets[f.key] = Offset(
                                                    oldOffset.dx +
                                                        details.localFocalPoint.dx-100,
                                                    oldOffset.dy +
                                ...
                                child: Stack(
                                  children: List<Widget>.generate(
                                    ...
                                    (int index) => TextOverlay(
                                      textOverlayModel:
                                          textOverlays.elementAt(index),
                                      updateCallback: () => setState(() {}),
                                      devicePadding: EdgeInsets
                                          .zero, //MediaQuery.of(context).padding,
                                      onTap: (textOverlay) =>
    ...
    var future = showModalBottomSheet<void>(
      ...
      builder: (BuildContext context) {
        return Container(
          ...
          child: Column(
            children: [
              Center(child: Text('Select Image Options')),
  ...
  static Future<File> _blendMedia({
    @required File mediaFile,
    @required File overlay,
  }) async {
    final ffmpeg = FlutterFFmpeg();
    final outputPath =
        mediaFile.path.substring(0, mediaFile.path.lastIndexOf('.')) +
            '-blended.mp4';
    print(
        'ffmpeg -y -i \"${mediaFile.path}\" -i \"${overlay.path}\" -filter_complex \"[0:v][1:v] overlay=0:0\" -c:v mpeg4 -c:a copy \"$outputPath\"');
    await ffmpeg.execute(// -pix_fmt yuv420p
        '-y -i \"${mediaFile.path}\" -i \"${overlay.path}\"  -filter_complex \"[1:v]scale=$width:$height [ovrl],[0:v][ovrl]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2\" -qscale 0 -c:v mpeg4 -c:a copy \"$outputPath\"');
